...
class Context {
  ...
  constructor() {
    let _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
      _ref$sharedKey = _ref.sharedKey,
      sharedKey = _ref$sharedKey === void 0 ? false : _ref$sharedKey;
  ...
  async _decryptFrame(encodedFrame, keyIndex) {
    ...
    const encryptionKey = keyData.encryptionKey;
    let material = keyData.material;
...
onmessage = event => {
  ...
  if (operation === 'initialize') {
    ...
    const _event$data = event.data,
      readableStream = _event$data.readableStream,
      writableStream = _event$data.writableStream,
      participantId = _event$data.participantId;
    ...
    const _event$data2 = event.data,
      participantId = _event$data2.participantId,
      key = _event$data2.key,
      keyIndex = _event$data2.keyIndex;
...
if (self.RTCTransformEvent) {
  self.onrtctransform = event => {
    ...
    const _transformer$options = transformer.options,
      operation = _transformer$options.operation,
      participantId = _transformer$options.participantId;
    const context = getParticipantContext(participantId);
