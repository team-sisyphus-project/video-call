...
// RSA key pairs for the ASAP (RS256) test tokens are generated at test time
// rather than committed to the repository, so no private key material lives in
// version control. Each pair's PUBLIC key is written to the fixtures directory
// (test-asap-public.pem / test-system-asap-public.pem); the Prosody test image
// copies those public keys and serves them via mod_test_observer_http, so the
// key the tokens are signed with always matches the key Prosody verifies with.
//
// IMPORTANT: this module must be loaded before the Prosody test image is built
// (e.g. from a pretest step), so the freshly generated public keys are the ones
// baked into the image.
function generateAsapKeyPair(publicKeyFixture) {
    ...
    fs.writeFileSync(path.join(__dirname, '../fixtures', publicKeyFixture), publicKey, 'utf8');

    return privateKey;
}

// RSA key pair for login ASAP (RS256) tokens.
...
export const ASAP_PRIVATE_KEY = generateAsapKeyPair('test-asap-public.pem');
...
export const SYSTEM_ASAP_PRIVATE_KEY = generateAsapKeyPair('test-system-asap-public.pem');
